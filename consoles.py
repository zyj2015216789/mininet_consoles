...
class ConsoleApp( Frame ):
    ...
    def ping( self ):
        ...
        for console in consoles:
            i = ( i + 1 )% count
            name = consoles[ i ].node.name
            console.sendCmd( 'sudo ./bgf -i '+ name+'-eth0 ' + ipList )
...
if __name__ == '__main__':
    ...
    controller1 = RemoteController(name='c0', ip='192.168.55.6', port=6653 )
    network = Mininet(topo=tree4, controller=controller1)
    network.start()
